...
1. Скачайте `ScreenOff-1.0.3-arm64.dmg` из [Releases](../../releases/latest). При обновлении сначала завершите старый ScreenOff кнопкой питания в меню.
...
- Перед сном и при закрытии крышки ScreenOff запрашивает восстановление. Если экран временно недоступен, попытки продолжаются после пробуждения и открытия крышки. До подтверждённого включения повторное отключение заблокировано. Автоматическое отключение возобновляется при следующем подключении внешнего монитора; вручную переключить экран можно сразу после завершения восстановления.
...
Версия 1.0.3 учитывает случай, когда macOS полностью убирает встроенный дисплей из списка и создаёт виртуальный экран-заглушку. Приложение исключает такую заглушку из внешних мониторов и для восстановления использует последний подтверждённый ID встроенного экрана. Этот запасной путь разрешён только для включения, при открытой крышке и отсутствии рабочего внешнего дисплея; новый обнаруженный ID всегда имеет приоритет. Это дополнительные механизмы восстановления, а не гарантия от сбоев самой macOS.
